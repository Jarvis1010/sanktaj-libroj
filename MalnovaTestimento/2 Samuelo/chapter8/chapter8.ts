export const chapter8 = {
  chapterTitle: "Ĉaptiro 8",
  summary: "",
  verses: [
    "Post tio okazis, ke David venkobatis la Filiŝtojn kaj humiligis ilin, kaj David prenis la ĉefurbon el la manoj de la Filiŝtoj.",
    "Li ankaŭ venkobatis la Moabidojn, kaj, kuŝiginte ilin sur la tero, li mezuris ilin per ŝnuro; du partojn li mezuris por mortigi, kaj unu plenan mezuron por restigi vivaj. Kaj la Moabidoj submetiĝis al David kaj alportis tributojn.",
    "Kaj David venkobatis Hadadezeron, filon de ReĤob, reĝon de Coba, kiam tiu iris, por restarigi sian regadon super la rivero Eŭfrato.",
    "Kaj David venkoprenis de li mil sepcent rajdantojn kaj dudek mil piedirantojn; kaj David lamigis ĉiujn ĉarĉevalojn, sed restigis el ili por cent ĉaroj.",
    "La Sirianoj Damaskaj venis, por helpi al Hadadezer, reĝo de Coba; sed David venkobatis el la Sirianoj dudek du mil homojn.",
    "Kaj David restigis garnizonojn en la Damaska Sirio, kaj la Sirianoj submetiĝis al David kaj alportis tributojn. Kaj la Eternulo helpis al David ĉie, kien li iris.",
    "Kaj David prenis la orajn ŝildojn, kiujn havis sur si la servantoj de Hadadezer, kaj alportis ilin en Jerusalemon.",
    "Kaj el BetaĤ kaj el Berotaj, urboj de Hadadezer, la reĝo David prenis tre multe da kupro.",
    "Kiam Toi, reĝo de ĥamat, aŭdis, ke David venkobatis la tutan militistaron de Hadadezer,",
    "tiam Toi sendis sian filon Joram al la reĝo David, por saluti lin, kaj gratuli lin pro tio, ke li militis kontraŭ Hadadezer kaj venkobatis lin (ĉar Toi estis en milito kontraŭ Hadadezer); kaj li havis en siaj manoj vazojn arĝentajn kaj vazojn orajn kaj vazojn kuprajn.",
    "Ilin ankaŭ la reĝo David dediĉis al la Eternulo, kune kun la arĝento kaj oro, kiun li dediĉis el tio, kion li akiris de ĉiuj nacioj, kiujn li venkis:",
    "de la Sirianoj, de Moab, de la Amonidoj, de la Filiŝtoj, kaj de Amalek, kaj el la militakiro, kiun li ricevis de Hadadezer, filo de ReĤob kaj reĝo de Coba.",
    "Kaj David faris al si nomon, kiam li revenis post la venkobato de la Sirianoj en la Valo de Salo, en la nombro de dek ok mil homoj.",
    "Kaj li starigis en Edomujo garnizonojn, en la tuta Edomujo li starigis garnizonojn; kaj ĉiuj Edomidoj submetiĝis al David. Kaj la Eternulo helpis al David ĉie, kien li iris.",
    "Kaj David reĝis super la tuta Izrael, kaj David faradis juĝon kaj justecon al sia tuta popolo.",
    "Joab, filo de Ceruja, estis estro de la militistaro; Jehoŝafat, filo de AĤilud, estis kronikisto;",
    "Cadok, filo de AĤitub, kaj AĤimeleĤ, filo de Ebjatar, estis pastroj; kaj Seraja estis skribisto;",
    "Benaja, filo de Jehojada, estis super la Keretidoj kaj la Peletidoj; kaj la filoj de David estis pastroj.",
  ],
} as const satisfies Chapter;
